...
const codexRateLimitWindows = (id: string, label: string, entry: Json): ModelUsage[] => {
  ...
  if (isObject(entry.primary_window)) {
    windows.push({ id: `${id}:5h`, label: `${label} · 5h`, limit: codexWindowFrom(entry.primary_window) });
  }
  if (isObject(entry.secondary_window)) {
    windows.push({ id: `${id}:7d`, label: `${label} · 7d`, limit: codexWindowFrom(entry.secondary_window) });
  }
  return windows;
};
...
const codexModelBreakdown = (raw: Json): ModelUsage[] => {
  ...
  if (Array.isArray(additional)) {
    additional.forEach((entry, index) => {
      ...
      const label = humanizeSlug(firstString(entry.model, entry.name, entry.label) ?? `limit_${index + 1}`);
