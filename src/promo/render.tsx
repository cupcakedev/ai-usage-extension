...
const Headline = ({ size, lines = HEADLINE }: { size: number; lines?: readonly string[] }) => (
...
const Sub = ({ lines, size }: { lines: readonly string[]; size: number }) => (
  <p className="promo-sub" data-fit={size} style={{ fontSize: size }}>
    {lines.map((line) => (
      <span key={line}>{line}</span>
    ))}
  </p>
...
function markClippedWindows(): void {
  document.querySelectorAll<HTMLElement>('.promo-window').forEach((element) => {
    if (element.scrollHeight > element.clientHeight + 2) {
      element.classList.add('promo-window--fade');
    }
  });
}
  ...
  <Frame width={CANVAS.tile.width} height={CANVAS.tile.height} center>
    ...
    >
      ...
      <div style={{ marginTop: 18 }}>
        <Headline size={TILE.headlineSize} lines={[BRAND]} />
      </div>
